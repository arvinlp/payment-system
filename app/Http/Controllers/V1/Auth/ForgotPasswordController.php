 ...
 * @Date: 2024-08-19 13:54:22 
 * Copyright by Arvin Loripour 
 * WebSite : http://www.arvinlp.ir 
 * @Last Modified by:   Arvin.Loripour 
 * @Last Modified time: 2024-08-19 13:54:22 
 */
...
class ForgotPasswordController extends Controller{
    ...
    public function index(Request $request){
        ...
        return view('auth.forgot-password', ['mobile' => $mobile]);
    }
    ...
    public function sendNewPassword(Request $request){
        ...
        if(!self::checkMobile($mobile)){
            return redirect()->route('auth.forgot.password',['mobile'=>$mobile])->with('error' , __('شماره موبایل قابل قبول نمی‌باشد !'));
        }
    ...
    public function resendCode($mobile){
        ...
        if(env('APP_ENV') == 'local'){
            ...
            if($this->sms->sendPassword((float)$mobile, $name, $newPassword)){
                return redirect()->route('auth.login.pass',['mobile'=>$mobile])->with('success' , __('گذرواژه جدید برای شما ارسال شد.'));
            }else{
                return redirect()->route('auth.forgot.password',['mobile'=>$mobile])->with('error' , __('خطای در ارسال رخ داده است !'));
            }
        }
